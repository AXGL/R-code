...
season_stats <-read.csv("../input/nba-players-stats/Seasons_Stats.csv") 
salaries <- data.frame(read_excel("../input/nba-player-salary-19902017/Player - Salaries per Year (1990 - 2017).xlsx"))
```
...
kable(leveneTest(Salary.in..  ~ Pos, data = subset2))
```
...
```{r}
# homocedasticity test
kable(leveneTest(PPG  ~ Pos, data = subset2))
```
...
```{r}
# homocedasticity test
kable(leveneTest(APG  ~ Pos, data = subset2))
```
...
```{r}
kable(leveneTest(WS  ~ Pos, data = subset2))
```
...
### Further analysis with WS, APG and MPG. {.tabset .tabset-fade .tabset-pills}
